...
def test_retry_with_exponential_backoff_retry():
    ...
    decorated = retry_with_exponential_backoff(
        max_retries=3, base_delay=0.1, logger=mock_logger
    )(mock_func)
...
def test_retry_with_exponential_backoff_delay():
    ...
    with patch("time.sleep", mock_sleep):
        decorated = retry_with_exponential_backoff(
            max_retries=3, base_delay=1.0, max_delay=4.0
        )(mock_func)
...
def test_retry_on_rate_limit_with_logger():
    ...
    decorated = retry_on_rate_limit(max_retries=2, logger=mock_logger)(mock_func)
